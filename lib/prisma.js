...
export async function updateUserDiscounts(userId, usedDiscounts) {
  return prisma.user.update({
    where: { id: userId },
    data: { usedDiscounts: JSON.stringify(usedDiscounts) },
  });
}

// Booking functions
export async function createBooking(userId, bookingData) {
  // Если userId есть - сохраняем с пользователем
  // Если нет (неавторизованный) - сохраняем без userId
  if (userId) {
...
export async function getBookingById(id) {
  return prisma.booking.findUnique({ where: { id } , include: { user: true }});
}
...
export async function getBookingsByBarberAndDate(barber, date) {
  return prisma.booking.findMany({
    where: {
      barber,
      date,
